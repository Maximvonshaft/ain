...
1. 框架会自动根据运行时的 `SCRIPT_NAME`/`PHP_SELF` 推断子目录，例如当入口位于 `/memo/public/index.php` 时会识别 `/memo` 作为前缀。
2. 若服务器无法正确传递这些变量，可在 `.env` 中将 `APP_URL` 设置为完整地址（例如 `https://domain.com/memo`），或额外指定 `APP_BASE_PATH=memo` 强制前缀。
