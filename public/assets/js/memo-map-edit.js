(function(){
      ...
      class SimpleMind {
        ...
        setupPan(){
          ...
          const startPan=(evt)=>{
            ...
            if(allowPan){
              this.updatePointerPanState(true);
              if(isMiddle || isPrimary){ evt.preventDefault(); }
            }else{
              this.updatePointerPanState(false);
            }
          };
          const movePan=(evt)=>{
            ...
            if(isTouchLike){
              ...
              if(this.activePointers.size>=2){
                ...
                const offsetChanged=(Math.abs(nextOffsetX-this.offsetX)>0.1) || (Math.abs(nextOffsetY-this.offsetY)>0.1);
                if(scaleChanged || offsetChanged){
                  this.scale=nextScale;
                  this.offsetX=nextOffsetX;
                  this.offsetY=nextOffsetY;
                  this.applyTransform();
                  pinch.baseScale=this.scale;
                  pinch.initialDistance=distance;
                  pinch.originX=(centerX - this.offsetX)/this.scale;
                  pinch.originY=(centerY - this.offsetY)/this.scale;
                }
                return;
              }
        ...
        isWheelPinchGesture(evt){
          if(!evt) return false;
          if(evt.ctrlKey || evt.metaKey) return true;
          const deltaZ=typeof evt.deltaZ==='number'?Math.abs(evt.deltaZ):0;
          if(deltaZ>0.0001) return true;
          return false;
        }
        isLikelyTrackpadScroll(evt,isPinchGesture){
          if(!evt || isPinchGesture) return false;
          if(evt.deltaMode!==0) return false;
        ...
        handleWheel(evt){
          ...
          const isPinchGesture=this.isWheelPinchGesture(evt);
          const isTrackpadScroll=this.isLikelyTrackpadScroll(evt,isPinchGesture);
          const shouldPanTrackpad=isTrackpadScroll && (evt.shiftKey || evt.altKey);
          if(shouldPanTrackpad){
            const panFactor=evt.deltaMode===1?18:(evt.deltaMode===2?240:1);
            this.offsetX-=deltaX*panFactor;
            this.offsetY-=deltaY*panFactor;
            this.applyTransform();
            return;
          }
          const dominantDelta=(absDeltaY>=absDeltaX)? (deltaY) : (deltaX);
          const delta=dominantDelta!==0?dominantDelta:(deltaZ!==0?deltaZ:(deltaY));
          if(delta===0) return;
      ...
      const memoMapConfig=window.memoMapConfig||{};
      const cloneData=value=>{
        if(value===null||value===undefined) return value;
        try{ return JSON.parse(JSON.stringify(value)); }catch(_){ return value; }
      };
      const defaultData=cloneData(memoMapConfig.defaultData)||{};
      let initialData=cloneData(memoMapConfig.initialData);
      ...
      const initialAssetMeta = cloneData(memoMapConfig.initialAssetMeta) || {};
    ...
    const jm=new jsMind({
      ...
      const titleWidthMedia=window.matchMedia('(max-width: 720px)');
      let titleSizer=null;
      let titleResizeScheduled=false;
      function ensureTitleSizer(){
        if(titleSizer || !titleInput) return;
        titleSizer=document.createElement('span');
        titleSizer.className='map-title-sizer';
        document.body.appendChild(titleSizer);
      }
      function calculateTitleWidth(){
        if(!titleInput) return 0;
        ensureTitleSizer();
        if(!titleSizer) return 0;
        const computed=getComputedStyle(titleInput);
        titleSizer.style.font=computed.font;
        titleSizer.style.letterSpacing=computed.letterSpacing;
        titleSizer.style.textTransform=computed.textTransform;
        const rawValue=titleInput.value || titleInput.placeholder || '';
        titleSizer.textContent=rawValue || '';
        const paddingLeft=parseFloat(computed.paddingLeft)||0;
        const paddingRight=parseFloat(computed.paddingRight)||0;
        const borderLeft=parseFloat(computed.borderLeftWidth)||0;
        const borderRight=parseFloat(computed.borderRightWidth)||0;
        const baseWidth=Math.ceil(titleSizer.getBoundingClientRect().width);
        const buffer=12;
        const width=baseWidth + paddingLeft + paddingRight + borderLeft + borderRight + buffer;
        const minWidth=240;
        const maxWidth=540;
        return Math.max(minWidth, Math.min(maxWidth, width));
      }
      function applyTitleInputWidth(){
        if(!titleInput) return;
        if(infoBarCollapsed || (titleWidthMedia && titleWidthMedia.matches)){
          titleInput.style.width='';
          return;
        }
        const nextWidth=calculateTitleWidth();
        if(nextWidth>0){
          titleInput.style.width=`${Math.round(nextWidth)}px`;
        }
      }
      function scheduleTitleWidth(){
        if(titleResizeScheduled) return;
        titleResizeScheduled=true;
        requestAnimationFrame(()=>{
          titleResizeScheduled=false;
          applyTitleInputWidth();
        });
      }
      const handleTitleMediaChange=()=>scheduleTitleWidth();
      if(titleWidthMedia){
        if(typeof titleWidthMedia.addEventListener==='function'){
          titleWidthMedia.addEventListener('change', handleTitleMediaChange);
        }else if(typeof titleWidthMedia.addListener==='function'){
          titleWidthMedia.addListener(handleTitleMediaChange);
        }
      }
      window.addEventListener('resize', scheduleTitleWidth, {passive:true});
      if(document.fonts && document.fonts.ready){
        document.fonts.ready.then(()=>scheduleTitleWidth()).catch(()=>{});
      }
