(function(){
      ...
      const currentItemId=timeline && timeline.dataset ? timeline.dataset.item || '' : '';
      ...
      if(itemFileInput){
        itemFileInput.addEventListener('change', async (e)=>{
          ...
          const fd=new FormData(); fd.append('action','upload_attachment'); fd.append('target','item'); fd.append('target_id', currentItemId || ''); fd.append('file', f);
      ...
      (function(){ // DnD steps
        ...
        box.addEventListener('drop',e=>{
          ...
          fetch(location.href,{method:'POST',body:fd,headers:{'X-Requested-With':'fetch'}});
            ...
            document.querySelectorAll('form[id^="form-notes-"]').forEach(form=>{
        const idInput=form.querySelector('input[name="id"]');
        const textarea=form.querySelector('textarea[name="notes"]');
        if(!idInput || !textarea) return;
        const stepId=idInput.value;
        const previewEl=document.getElementById(`step-md-view-${stepId}`);
        if(previewEl){ previewEl.innerHTML=safeHTML(textarea.value); }
      });
