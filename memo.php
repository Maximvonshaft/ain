    ...
    <script>
      (function(){
        ...
        const showLoading=(message)=>{
          ...
          const textEl=loadingEl.querySelector('.attachment-preview-loading-text');
          if(textEl){ textEl.textContent=message || '载入中…'; }
          bodyEl.innerHTML='';
          bodyEl.appendChild(loadingEl);
        };
        ...
        const loadZipPreview=async source=>{
          const attempt=async password=>{
            showLoading(password?'验证密码…':'解析压缩包…');
  ...
  <head>
    ...
    <style>
      ...
      *,*::before,*::after{box-sizing:border-box}
      ...
      .attachment-preview-loading{position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px;color:var(--text-muted);font:14px/1.6 'Inter','Noto Sans SC',sans-serif;letter-spacing:.08em}
      .attachment-preview-spinner{width:48px;height:48px;border-radius:50%;border:3px solid rgba(201,168,106,.24);border-top-color:var(--gold-400);animation:attachment-preview-spin 1s linear infinite}
      .attachment-preview-loading-text{letter-spacing:.12em;text-transform:uppercase}
        ...
        function buildRow(item,index){
          ...
          if(item.nodeTopic){
            ...
            nodeBtn.addEventListener('click',evt=>{
              evt.stopPropagation();
              close();
              setTimeout(()=>focusAttachmentNode(item.nodeId),0);
            });
