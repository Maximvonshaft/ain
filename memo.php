...
if ($view === 'map_edit') {
  ...
  <body>
    ...
    <script>
      ...
      function buildTraceRoute(start,end,directionHint=1,orientation='horizontal'){
        if(!start || !end){ return []; }
      ...
      class SimpleMind {
        ...
        updateLinkPath(node){
          ...
          const parent=node.parent;
          const dx=node.absX - parent.absX;
          const dy=node.absY - parent.absY;
          const prioritizeVertical=Math.abs(dy) > Math.abs(dx)*1.1;
          let start;
          let end;
          let directionHint=1;
          ...
          if(prioritizeVertical){
            const isAbove=dy<0;
            start=isAbove ? parent.anchors.top : parent.anchors.bottom;
            end=isAbove ? node.anchors.bottom : node.anchors.top;
            directionHint=isAbove?-1:1;
            orientation='vertical';
          }else{
            const isLeft=dx<0;
            start=isLeft ? parent.anchors.left : parent.anchors.right;
            end=isLeft ? node.anchors.right : node.anchors.left;
            directionHint=isLeft?-1:1;
          }
