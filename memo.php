...
if ($view === 'map_edit') {
  ...
  <body>
    ...
    <script>
      ...
      function buildAxisTraceRoute(start,end,axis='horizontal',directionHint=1){
        if(axis==='vertical'){
          const swappedStart={x:start.y,y:start.x};
          const swappedEnd={x:end.y,y:end.x};
          const swappedRoute=buildTraceRoute(swappedStart, swappedEnd, directionHint);
          return swappedRoute.map(pt=>({x:pt.y,y:pt.x}));
        }
        return buildTraceRoute(start,end,directionHint);
      }
      ...
      class SimpleMind {
        ...
        updateLinkPath(node){
          ...
          if(absDx>=absDy){
            ...
            route=buildAxisTraceRoute(start,end,'horizontal',isLeft?-1:1);
          }else{
            const isAbove=dy<0;
            start=isAbove ? parent.anchors.top : parent.anchors.bottom;
            end=isAbove ? node.anchors.bottom : node.anchors.top;
            route=buildAxisTraceRoute(start,end,'vertical',isAbove?-1:1);
          }
