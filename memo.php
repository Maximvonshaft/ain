  ...
  <head>
    ...
    <style>
      ...
      *,*::before,*::after{box-sizing:border-box}
      ...
      @keyframes scan{0%{transform:translateY(-100%)}100%{transform:translateY(100%)}}
      a{color:inherit;text-decoration:none}
      .mind-shell{min-height:100vh;display:flex;flex-direction:column;align-items:center;padding-bottom:140px}
      .editor-pane{position:relative;width:100%;max-width:1440px;margin:0 auto;flex:1;display:flex;flex-direction:column;padding:32px clamp(24px,5vw,48px) 0}
      .mind-hud{display:flex;align-items:flex-start;justify-content:space-between;gap:24px;margin-bottom:18px;background:linear-gradient(180deg,rgba(21,26,30,.92),rgba(15,19,22,.94));border:1px solid rgba(201,168,106,.28);border-radius:22px;padding:18px 22px;box-shadow:0 18px 40px rgba(0,0,0,.55),inset 0 0 0 1px rgba(201,168,106,.08);backdrop-filter:blur(14px)}
      .hud-left{display:flex;align-items:center;gap:18px;flex-wrap:wrap}
      .hud-back{display:inline-flex;align-items:center;gap:6px;padding:10px 14px;border-radius:12px;border:1px solid rgba(201,168,106,.28);background:rgba(12,16,18,.78);color:var(--gold-400);font:600 12px/1 'Inter','Noto Sans SC',sans-serif;letter-spacing:.14em;text-transform:uppercase;transition:border-color var(--transition),transform var(--transition);touch-action:manipulation}
      .hud-back:hover{transform:translateY(-1px);border-color:rgba(227,198,139,.6)}
      .hud-back:active{transform:translateY(0);border-color:var(--gold-700)}
      .hud-meta{display:flex;flex-direction:column;gap:6px;min-width:220px}
      .hud-meta input{padding:12px 16px;border-radius:14px;border:1px solid rgba(201,168,106,.32);background:rgba(12,16,18,.7);color:var(--text-strong);font:600 18px/1.4 'Cinzel','Noto Serif SC',serif;letter-spacing:.06em;text-transform:uppercase;transition:border-color var(--transition),box-shadow var(--transition)}
      .hud-meta input:focus{border-color:var(--gold-500);box-shadow:0 0 0 3px rgba(227,198,139,.18);outline:none}
      .hud-info{color:var(--text-muted);font:500 12px/1.6 'Inter','Noto Sans SC',sans-serif;letter-spacing:.12em;text-transform:uppercase}
      .hud-right{display:flex;align-items:center;gap:12px;margin-left:auto}
      .mind-stage{position:relative;flex:1;min-height:calc(100vh - 220px)}
      #jsmind-container{position:absolute;inset:0;width:100%;height:100%;overflow:hidden;background:linear-gradient(160deg,rgba(21,26,30,.82),rgba(10,12,14,.94));box-shadow:inset 0 0 48px rgba(0,0,0,.6);touch-action:none}
      .map-toolbar{position:absolute;top:24px;right:24px;display:flex;gap:12px;z-index:60}
      .map-toolbar button{position:relative;display:inline-flex;align-items:center;justify-content:center;padding:10px 16px;border-radius:var(--r-sm);border:1px solid rgba(201,168,106,.38);background:rgba(12,16,18,.86);color:var(--gold-400);font:600 12px/1 'Inter','Noto Sans SC',sans-serif;letter-spacing:.14em;text-transform:uppercase;cursor:pointer;transition:transform var(--transition),border-color var(--transition);touch-action:manipulation}
      .map-toolbar button:hover{transform:translateY(-2px);border-color:rgba(227,198,139,.65)}
      .map-toolbar button:active{transform:translateY(0);border-color:var(--gold-700)}
      ...
      .dock-wrap{position:fixed;left:50%;bottom:clamp(18px,4vh,32px);transform:translateX(-50%);z-index:95;pointer-events:none}
      .dock{pointer-events:auto;display:flex;align-items:center;gap:10px;padding:12px;background:linear-gradient(180deg,rgba(21,26,30,.9),rgba(12,16,18,.86));border:1px solid rgba(201,168,106,.28);border-radius:28px;box-shadow:0 18px 48px rgba(0,0,0,.55),0 0 28px rgba(227,198,139,.12) inset;backdrop-filter:blur(12px);transition:box-shadow var(--transition)}
      .dock-btn{position:relative;isolation:isolate;display:grid;place-items:center;width:88px;height:56px;border-radius:18px;border:1px solid rgba(201,168,106,.38);background:rgba(201,168,106,.08);color:var(--gold-400);font:600 13px/1 'Inter','Noto Sans SC',sans-serif;letter-spacing:.1em;text-transform:uppercase;cursor:pointer;transition:border-color var(--transition),background-color var(--transition);touch-action:manipulation;outline:none;--dock-scale:1;--dock-translate:0;transform:translateY(var(--dock-translate)) scale(var(--dock-scale))}
      .dock-btn svg{width:22px;height:22px}
      .dock-btn .label{position:absolute;bottom:6px;font:500 12px/1 'Inter','Noto Sans SC',sans-serif;color:var(--text-dim);letter-spacing:.08em;pointer-events:none}
      .dock-btn:hover{--dock-translate:-2px;border-color:var(--gold-500);background:rgba(201,168,106,.14)}
      .dock-btn:active{--dock-translate:0;border-color:var(--gold-700)}
      .dock-btn:focus-visible{box-shadow:0 0 0 3px rgba(227,198,139,.22),0 0 0 5px rgba(75,195,209,.28)}
      .dock-btn.danger{color:#f6d6d6;border-color:rgba(209,75,75,.45);background:rgba(209,75,75,.12)}
      .dock-btn.danger:hover{background:rgba(209,75,75,.18);border-color:rgba(209,75,75,.62)}
      .dock-btn.ghost{background:rgba(201,168,106,.05)}
      .dock-sep{width:10px;height:42px;border-right:1px solid rgba(201,168,106,.22);opacity:.7}
      .dock-more{position:relative}
      .dock-menu{position:absolute;bottom:64px;right:0;min-width:176px;padding:10px;margin:0;list-style:none;background:linear-gradient(180deg,rgba(21,26,30,.96),rgba(12,16,18,.92));border:1px solid rgba(201,168,106,.28);border-radius:16px;box-shadow:0 22px 48px rgba(0,0,0,.55);display:none}
      .dock-menu[aria-hidden="true"]{display:none}
      .dock-more[aria-expanded="true"] .dock-menu{display:block}
      .dock-menu li{margin:0;padding:0}
      .dock-menu button{width:100%;padding:10px 12px;border-radius:12px;border:0;background:transparent;color:var(--text-strong);font:600 13px/1.4 'Inter','Noto Sans SC',sans-serif;letter-spacing:.08em;text-align:left;cursor:pointer;transition:background-color var(--transition),color var(--transition)}
      .dock-menu button:hover{background:rgba(201,168,106,.12);color:var(--gold-400)}
      .dock[data-fisheye="on"] .dock-btn{transform-origin:50% 60%}
      .node-popover{position:fixed;z-index:110;min-width:280px;max-width:360px;background:linear-gradient(180deg,rgba(21,26,30,.96),rgba(12,16,18,.92));border:1px solid rgba(201,168,106,.28);border-radius:18px;box-shadow:0 28px 60px rgba(0,0,0,.55),inset 0 0 24px rgba(227,198,139,.08);padding:18px 20px;display:flex;flex-direction:column;gap:14px;backdrop-filter:blur(16px)}
      .node-popover[hidden]{display:none}
      .node-popover__header{display:flex;align-items:center;justify-content:space-between;gap:12px}
      .node-popover__header h2{margin:0;font:600 16px/1.4 'Cinzel','Noto Serif SC',serif;color:var(--gold-400);letter-spacing:.08em;text-transform:uppercase}
      .node-popover__close{border:0;background:transparent;color:var(--text-dim);font-size:22px;cursor:pointer;padding:6px;border-radius:10px;transition:color var(--transition),background-color var(--transition)}
      .node-popover__close:hover{color:var(--gold-400);background:rgba(201,168,106,.12)}
      .node-popover__body{display:grid;gap:12px}
      .node-popover__body .row{display:grid;gap:8px}
      .node-popover__body label{font:600 12px/1.4 'Inter','Noto Sans SC',sans-serif;color:var(--text-muted);letter-spacing:.14em;text-transform:uppercase}
      .node-popover__body input,.node-popover__body select{width:100%;padding:12px 14px;border-radius:12px;border:1px solid rgba(201,168,106,.28);background:rgba(12,16,18,.74);color:var(--text-strong);font:500 14px/1.5 'Noto Sans SC','Inter',sans-serif;letter-spacing:.02em;transition:border-color var(--transition),box-shadow var(--transition)}
      .node-popover__body input:focus,.node-popover__body select:focus{border-color:var(--gold-500);box-shadow:0 0 0 3px rgba(227,198,139,.18);outline:none}
      .node-popover__actions{display:flex;justify-content:flex-end;gap:10px}
      .btn-popover{padding:10px 16px;border-radius:12px;border:1px solid rgba(201,168,106,.32);background:rgba(12,16,18,.78);color:var(--gold-400);font:600 13px/1 'Inter','Noto Sans SC',sans-serif;letter-spacing:.1em;text-transform:uppercase;cursor:pointer;transition:transform var(--transition),border-color var(--transition)}
      .btn-popover:hover{transform:translateY(-1px);border-color:rgba(227,198,139,.6)}
      .btn-popover.primary{background:linear-gradient(135deg,rgba(201,168,106,.24),rgba(170,140,84,.28));color:var(--bg-void)}
      .sr-only{position:absolute;width:1px;height:1px;padding:0;margin:-1px;overflow:hidden;clip:rect(0,0,0,0);white-space:nowrap;border:0}
      .save-tip{font:600 12px/1 'Inter','Noto Sans SC',sans-serif;letter-spacing:.16em;color:var(--gold-400);text-transform:uppercase;display:none}
      .save-tip.dirty{color:var(--accent-crimson)}
      .save-tip.show{display:inline-flex}
      @media (max-width:960px){
        .editor-pane{padding:24px 20px 0}
        .mind-hud{flex-direction:column;align-items:flex-start;gap:16px}
      ...
      @media (prefers-reduced-motion: reduce){
        ...
        .dock[data-fisheye="on"] .dock-btn{transform:none !important}
      }
  ...
  <body>
    ...
    <div class="mind-shell">
      <main class="editor-pane">
        <header class="mind-hud">
          <div class="hud-left">
            <a class="hud-back" href="<?= htmlspecialchars($_SERVER['PHP_SELF'].'?view=maps') ?>" aria-label="返回导图库">← 导图库</a>
            <div class="hud-meta">
              <input id="map-title" value="<?php echo h($mind['title']); ?>" placeholder="输入导图标题" aria-label="导图标题">
              <div class="hud-info">ID：<?php echo $mind['id'] ?: '新建'; ?> · 最近保存：<?php echo dt((int)$mind['updated_at']); ?></div>
            </div>
          </div>
          <div class="hud-right">
            <div class="save-tip" id="save-state" role="status" aria-live="polite">保存成功</div>
          </div>
        </header>
      ...
      <div class="dock-wrap" role="toolbar" aria-label="思维导图操作工具栏">
        <nav class="dock" id="mind-dock">
          ...
          <button class="dock-btn" data-action="sibling" title="新增同级 (Enter)" aria-label="新增同级">
          ...
          <button class="dock-btn" data-action="child" title="新增子级 (Tab)" aria-label="新增子级">
          ...
          <button class="dock-btn danger" data-action="delete" title="删除 (Del)" aria-label="删除" aria-describedby="dock-danger-hint">
          ...
          <div class="dock-more">
            <button class="dock-btn ghost" id="dock-more-toggle" aria-haspopup="menu" aria-expanded="false" title="更多">
            ...
            <ul class="dock-menu" id="mind-dock-menu" role="menu">
              <li role="menuitem">
                <button type="button" data-action="import" id="btn-import-json">导入 JSON</button>
              </li>
              <li role="menuitem">
                <button type="button" data-action="export" id="btn-export-json">导出 JSON</button>
              </li>
              <li role="menuitem">
                <button type="button" data-action="toggle-fold" id="btn-collapse-menu">折叠/展开</button>
              </li>
              <li role="menuitem">
                <button type="button" data-action="fit-view" id="btn-fit">自适应视图</button>
              </li>
              <li role="menuitem">
                <button type="button" data-action="center" id="btn-center">居中</button>
              </li>
              <li role="menuitem">
                <button type="button" data-action="zoom-in" id="btn-zoom-in">放大</button>
              </li>
              <li role="menuitem">
                <button type="button" data-action="zoom-out" id="btn-zoom-out">缩小</button>
              </li>
            </ul>
          </div>
        </nav>
        <p id="dock-danger-hint" class="sr-only">此操作不可撤销，请谨慎。</p>
      </div>
    </div>
    <div class="node-popover" id="node-inspector" hidden>
      <div class="node-popover__header">
        <h2>节点属性</h2>
        <button type="button" class="node-popover__close" aria-label="关闭节点属性">×</button>
      </div>
      <div class="node-popover__body">
        <div class="row">
      ...
      <div class="node-popover__actions">
        <button type="button" class="btn-popover" data-popover="cancel">取消</button>
        <button type="button" class="btn-popover primary" data-popover="apply">保存</button>
      </div>
    </div>
    ...
    <script>
      ...
      function buildTraceRoute(start,end,directionHint=1,axis='horizontal'){
        if(!start || !end){ return []; }
        ...
        if(axis==='vertical'){
          ...
          const direction=(absDy<TRACE_GRID*0.5)
            ? (directionHint>=0?1:-1)
            : (rawDy>=0?1:-1);
          let midY;
          if(absDy<TRACE_GRID*0.25){
            midY=start.y + direction*MIN_LEAD;
          }else if(absDy<=TRACE_GRID*3){
            const halfSpan=absDy/2;
            midY=start.y + direction*Math.max(MIN_LEAD, halfSpan);
          }else{
            const maxLead=Math.max(MIN_LEAD, absDy - TRACE_GRID*2);
            let lead=absDy*0.45;
            lead=Math.min(Math.max(MIN_LEAD, lead), maxLead);
            midY=start.y + direction*lead;
          }
          midY=alignToTraceGrid(midY);
          const viaX=alignToTraceGrid(end.x);
          points.push({x:start.x,y:midY});
          if(!nearlyEqual(viaX,start.x)){
            points.push({x:viaX,y:midY});
          }
      ...
      class SimpleMind {
        ...
        updateLinkPath(node){
          ...
          if(absDx>=absDy){
            const goLeft=dx<0;
            start=goLeft ? (parent.anchors.left||parent.anchors.center) : (parent.anchors.right||parent.anchors.center);
            end=goLeft ? (node.anchors.right||node.anchors.center) : (node.anchors.left||node.anchors.center);
            directionHint=goLeft?-1:1;
            orientation='horizontal';
          }else{
            const goUp=dy<0;
            start=goUp ? (parent.anchors.top||parent.anchors.center) : (parent.anchors.bottom||parent.anchors.center);
            end=goUp ? (node.anchors.bottom||node.anchors.center) : (node.anchors.top||node.anchors.center);
            directionHint=goUp?-1:1;
            orientation='vertical';
          }
          if(!start || !end){
            start=parent.anchors.center;
            end=node.anchors.center;
          }
          const route=buildTraceRoute(start,end,directionHint,orientation);
    ...
    const jm=new jsMind({
      ...
      jm.options.onInlineEdit=node=>openInspectorForNode(node,{focusTitle:true});
      jm.show(initialData);
      ...
      window.addEventListener('resize',()=>{
        requestAnimationFrame(()=>{
          ...
          if(inspectorVisible){ positionInspector(activeInspectorNode()); }
        });
      });
      ...
      const popoverClose=inspector ? inspector.querySelector('.node-popover__close') : null;
      const popoverApply=inspector ? inspector.querySelector('[data-popover="apply"]') : null;
      const popoverCancel=inspector ? inspector.querySelector('[data-popover="cancel"]') : null;
      const dock=document.getElementById('mind-dock');
      const dockMoreToggle=document.getElementById('dock-more-toggle');
      const dockMenu=document.getElementById('mind-dock-menu');
      const dockButtons=dock ? Array.from(dock.querySelectorAll('.dock-btn')) : [];
      const dockPrimaryButtons=dock ? dockButtons.filter(btn=>!btn.closest('.dock-menu')) : [];
      const dockMenuButtons=dockMenu ? Array.from(dockMenu.querySelectorAll('button[data-action]')) : [];
      const saveButton=dock ? dock.querySelector('.dock-btn[data-action="save"]') : null;
      const addSiblingButton=dock ? dock.querySelector('.dock-btn[data-action="sibling"]') : null;
      const addChildButton=dock ? dock.querySelector('.dock-btn[data-action="child"]') : null;
      const attachFileBtn=dock ? dock.querySelector('.dock-btn[data-action="attach"]') : null;
      const attachLinkBtn=dock ? dock.querySelector('.dock-btn[data-action="link"]') : null;
      const deleteButton=dock ? dock.querySelector('.dock-btn[data-action="delete"]') : null;
      ...
      const collapseMenuButton=document.getElementById('btn-collapse-menu');
      ...
      let saveButtonDefault=readButtonLabel(saveButton) || '保存';
      let dirty=false;
      const commandLog=[];
      ...
      function setSaveButtonState(text, disabled){
        if(typeof text==='string'){
          if(saveButton) setButtonLabel(saveButton, text);
        }else if(text===null){
          if(saveButton) setButtonLabel(saveButton, saveButtonDefault);
        }
        if(typeof disabled==='boolean' && saveButton){
      ...
      const inspectorFields=[nodeTitleInput,nodeTypeSelect,nodeStatusSelect,nodePrioritySelect,nodeOwnerInput,nodeTagsInput].filter(Boolean);
      let inspectorSyncing=false;
      let inspectorVisible=false;
      let inspectorNodeId=null;
      function escapedNodeSelector(id){
        if(!id) return null;
        const safe=typeof CSS!=='undefined' && CSS && typeof CSS.escape==='function' ? CSS.escape(id) : id.replace(/"/g,'\\"');
        return `.jsmind-node[nodeid="${safe}"]`;
      }
      function findNodeElement(node){
        const id=typeof node==='string'?node:(node?node.id:null);
        if(!id) return null;
        const selector=escapedNodeSelector(id);
        return selector ? document.querySelector(selector) : null;
      }
      function activeInspectorNode(){
        if(inspectorNodeId){
          const node=jm.get_node(inspectorNodeId);
          if(node) return node;
        }
        return jm.get_selected_node();
      }
      function positionInspector(node){
        if(!inspector || !inspectorVisible) return;
        const pad=18;
        const viewportWidth=window.innerWidth;
        const viewportHeight=window.innerHeight;
        const sheetMode=window.matchMedia('(max-width: 720px)').matches;
        if(sheetMode){
          const width=Math.min(viewportWidth - pad*2, 360);
          inspector.style.width=`${Math.round(width)}px`;
          inspector.style.left='50%';
          inspector.style.top='auto';
          inspector.style.bottom=`${Math.max(pad, 20)}px`;
        ...
        const target=node ? findNodeElement(node) : null;
        ...
        left=Math.min(viewportWidth-rect.width-pad, Math.max(pad, left));
        top=Math.min(viewportHeight-rect.height-pad, Math.max(pad, top));
        inspector.style.left=`${Math.round(left)}px`;
        inspector.style.top=`${Math.round(top)}px`;
      }
      function openInspectorForNode(node,{focusTitle=false}={}){
        if(!inspector || !node) return;
        inspectorNodeId=node.id;
        if(typeof jm.select_node==='function'){ jm.select_node(node.id); }
      ...
      function commitInspectorTitle(){
        if(inspectorSyncing || !nodeTitleInput) return;
        const node=activeInspectorNode();
        if(!node) return;
        const current=node.topic || '';
        const next=(nodeTitleInput.value || '').trim();
        if(!next || next===current) return;
        commitInlineEditing();
        jm.update_node(node.id, next);
        markDirty();
      ...
      function refreshInspector(node){
        ...
        if(nodeTitleInput) nodeTitleInput.value=node.topic || '';
        ...
        inspectorSyncing=false;
        if(inspectorVisible){ positionInspector(node); }
      }
      if(nodeTitleInput){
        nodeTitleInput.addEventListener('keydown',e=>{
          if(e.key==='Enter'){
            e.preventDefault();
            commitInspectorTitle();
            closeInspector();
          }
        });
        nodeTitleInput.addEventListener('blur',()=>{
          if(inspectorVisible){ commitInspectorTitle(); }
        });
      }
      if(popoverApply){
      ...
      if(popoverClose){ popoverClose.addEventListener('click',()=>closeInspector()); }
      document.addEventListener('pointerdown',evt=>{
        if(!inspectorVisible || !inspector) return;
        if(inspector.contains(evt.target)){ return; }
        if(nodeTitleInput && document.activeElement===nodeTitleInput){ commitInspectorTitle(); }
        closeInspector();
      }, true);
      ...
      function ensureNode(){
        let node=null;
        if(inspectorVisible && inspectorNodeId){
          node=jm.get_node(inspectorNodeId);
        }
        if(!node){ node=jm.get_selected_node(); }
        if(node) return node;
      ...
      if(dock && !coarsePointer && !prefersReducedMotion){
        ...
        dock.addEventListener('mousemove',e=>{
          ...
          dockPrimaryButtons.forEach(btn=>{
            ...
            const scale=Math.max(1, 1.18 - dist/720);
            btn.style.setProperty('--dock-scale', scale.toFixed(3));
          });
        });
        dock.addEventListener('mouseleave',()=>{
          dockPrimaryButtons.forEach(btn=>btn.style.setProperty('--dock-scale','1'));
        });
      }
      function setDockMenu(expanded){
        if(!dockMoreToggle || !dockMenu) return;
        dockMoreToggle.setAttribute('aria-expanded', String(expanded));
        const parent=dockMoreToggle.closest('.dock-more');
        if(parent){ parent.setAttribute('aria-expanded', String(expanded)); }
        dockMenu.setAttribute('aria-hidden', expanded ? 'false' : 'true');
      }
      if(dockMoreToggle){
        setDockMenu(false);
        dockMoreToggle.addEventListener('click',()=>{
          const expanded=dockMoreToggle.getAttribute('aria-expanded')==='true';
          setDockMenu(!expanded);
        });
      }
      document.addEventListener('pointerdown',evt=>{
        if(!dockMenu || !dockMoreToggle) return;
        if(dockMoreToggle.getAttribute('aria-expanded')!=='true') return;
        if(dockMenu.contains(evt.target) || dockMoreToggle.contains(evt.target)) return;
        setDockMenu(false);
      });
      ...
      function handleDockAction(action){
        switch(action){
          ...
          case 'export': if(exportButton){ exportButton.click(); } break;
          case 'toggle-fold':
            if(collapseMenuButton){ collapseMenuButton.blur(); }
            if(collapseButton){ collapseButton.click(); }
            else{ collapseCurrentNode(); }
            break;
          case 'fit-view': if(fitButton){ fitButton.click(); } break;
          case 'center': if(centerButton){ centerButton.click(); } break;
          case 'zoom-in': if(zoomInButton){ zoomInButton.click(); } break;
          case 'zoom-out': if(zoomOutButton){ zoomOutButton.click(); } break;
          default: break;
        }
      }
      if(dock){
        dock.addEventListener('click',e=>{
          ...
          if(!action) return;
          if(target.closest('.dock-more')){ setDockMenu(false); }
          handleDockAction(action);
        });
        dock.addEventListener('keydown',e=>{
          const current=e.target.closest('.dock-btn');
          if(!current) return;
          const index=dockPrimaryButtons.indexOf(current);
          if(index<0) return;
          if(e.key==='ArrowRight'){
            e.preventDefault();
            const next=dockPrimaryButtons[Math.min(index+1, dockPrimaryButtons.length-1)];
            next?.focus();
          }else if(e.key==='ArrowLeft'){
            e.preventDefault();
            const prev=dockPrimaryButtons[Math.max(index-1,0)];
            prev?.focus();
          }
        });
      }
      if(dockMenuButtons.length){
        dockMenuButtons.forEach(btn=>btn.addEventListener('click',()=>setDockMenu(false)));
      }
      document.addEventListener('keydown',e=>{
        if(inspectorVisible && e.key==='Escape'){
          e.preventDefault();
          closeInspector();
          return;
        }
