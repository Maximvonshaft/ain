  ...
  try{
    switch($action){
      ...
      case 'purge_mindmap_session_assets': {
        ...
        if($sessionKey==='') throw new RuntimeException('会话未初始化');
        $raw=$_POST['ids'] ?? [];
        $ids=[];
        if(is_string($raw)){
          $decoded=json_decode($raw,true);
        ...
        if($ids){
          $placeholders=implode(',',array_fill(0,count($ids),'?'));
          $params=$ids;
          array_unshift($params,$sessionKey);
          $st=$pdo->prepare("SELECT id FROM mindmap_assets WHERE session_key=? AND mindmap_id IS NULL AND id IN ($placeholders)");
          $st->execute($params);
        }else{
          $st=$pdo->prepare('SELECT id FROM mindmap_assets WHERE session_key=? AND mindmap_id IS NULL');
          $st->execute([$sessionKey]);
        }
        $deleted=0;
        foreach($st->fetchAll() as $row){
          delete_mindmap_asset((int)$row['id']);
          $deleted++;
        }
        header('Content-Type: application/json');
...
标签：#重要 #任务"></textarea>
    ...
    <script>
      ...
      const unsavedMindmapAssetIds=new Set();
      let unsavedMindmapAssetBeaconSent=false;
      function registerUnsavedMindmapAsset(assetId){
        const id=Number(assetId);
        if(!Number.isFinite(id) || id<=0) return;
        if(currentMapId>0){
          unsavedMindmapAssetIds.delete(id);
          return;
        }
        unsavedMindmapAssetIds.add(id);
        unsavedMindmapAssetBeaconSent=false;
      }
      function unregisterUnsavedMindmapAssets(values){
        if(!values) return;
        const list=Array.isArray(values)?values:[values];
        list.forEach(val=>{
          const id=Number(val);
          if(Number.isFinite(id) && id>0){ unsavedMindmapAssetIds.delete(id); }
        });
      }
      function clearUnsavedMindmapAssets(){
        unsavedMindmapAssetIds.clear();
        unsavedMindmapAssetBeaconSent=false;
      }
      function purgeUnsavedMindmapAssetsViaBeacon(){
        if(currentMapId>0 || unsavedMindmapAssetIds.size===0 || unsavedMindmapAssetBeaconSent) return;
        const ids=Array.from(unsavedMindmapAssetIds);
        if(!ids.length) return;
        unsavedMindmapAssetBeaconSent=true;
        ...
        if(typeof navigator!=='undefined' && navigator && typeof navigator.sendBeacon==='function'){
          try{
            ...
            sent=navigator.sendBeacon(location.href, blob);
          }catch(_){ }
        }
        if(!sent){
          try{
      ...
      if(Array.isArray(initialAssetMeta)){
        initialAssetMeta.forEach(entry=>{
          ...
          if(!currentMapId){ registerUnsavedMindmapAsset(id); }
        });
      }
    ...
    const jm=new jsMind({
      ...
      async function attachFilesToNode(targetNode, files){
        ...
        results.forEach((result,index)=>{
          ...
          if(result.status==='fulfilled'){
            ...
            registerUnsavedMindmapAsset(uploaded.id);
      ...
      function createAttachmentManager(){
        ...
        function handleDelete(ids){
          ...
          requestDelete(unique).then(()=>{
            ...
            refresh(true);
            unregisterUnsavedMindmapAssets(unique);
          }).catch(err=>{
        ...
        function onFilesUploaded(entries, targetNode){
          if(entries && entries.length){
            entries.forEach(entry=>{
              ...
              registerUnsavedMindmapAsset(entry.id);
            });
          }
      ...
      window.addEventListener('beforeunload',e=>{
        ...
        if(!dirty){
          purgeUnsavedMindmapAssetsViaBeacon();
          return;
        }
        e.preventDefault();
        e.returnValue='';
