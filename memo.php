  ...
  try{
    switch($action){
      ...
      case 'purge_mindmap_session_assets': {
        ...
        if($sessionKey===''){
          header('Content-Type: application/json');
          echo json_encode(['ok'=>1,'deleted'=>0]);
          exit;
        }
        $raw=$_POST['ids'] ?? [];
        $ids=[];
        ...
        if($ids){
          ...
          $params=$ids;
          $params[]=$sessionKey;
          $st=$pdo->prepare("SELECT id FROM mindmap_assets WHERE id IN ($placeholders) AND mindmap_id IS NULL AND session_key=?");
          $st->execute($params);
        }else{
          $st=$pdo->prepare('SELECT id FROM mindmap_assets WHERE mindmap_id IS NULL AND session_key=?');
...
标签：#重要 #任务"></textarea>
    ...
    <script>
      ...
      const pendingSessionAssetIds=new Set();
      function rememberPendingSessionAsset(id){
        const numeric=Number(id);
        if(Number.isFinite(numeric) && numeric>0){ pendingSessionAssetIds.add(numeric); }
      }
      function forgetPendingSessionAsset(id){
        const numeric=Number(id);
        if(Number.isFinite(numeric) && numeric>0){ pendingSessionAssetIds.delete(numeric); }
      }
      function collectPendingSessionAssetIds(){
        return Array.from(pendingSessionAssetIds.values());
      }
      if(Array.isArray(initialAssetMeta)){
        initialAssetMeta.forEach(entry=>{
          ...
          const mindmapId=Number(entry.mindmap_id);
          const sessionKey=typeof entry.session_key==='string'?entry.session_key.trim():'';
          if((!Number.isFinite(mindmapId) || mindmapId<=0) && sessionKey){ rememberPendingSessionAsset(id); }
        });
      }
      ...
      function purgePendingSessionAssets(ids){
        const source=Array.isArray(ids)?ids:collectPendingSessionAssetIds();
        const normalized=Array.from(new Set(source.map(id=>Number(id)).filter(num=>Number.isFinite(num) && num>0)));
        if(!normalized.length) return false;
        const params=new URLSearchParams();
        params.set('action','purge_mindmap_session_assets');
        params.set('ids', JSON.stringify(normalized));
        const body=params.toString();
        let dispatched=false;
        try{
          if(typeof navigator!=='undefined' && typeof navigator.sendBeacon==='function'){
            const payload=new Blob([body], { type:'application/x-www-form-urlencoded;charset=UTF-8' });
            dispatched=navigator.sendBeacon(location.href, payload);
          }
        }catch(err){
          dispatched=false;
        }
        ...
        if(dispatched){ normalized.forEach(id=>pendingSessionAssetIds.delete(id)); }
        return dispatched;
      }
    ...
    const jm=new jsMind({
      ...
      function markSaved(){
        dirty=false;
        if(Number.isFinite(currentMapId) && currentMapId>0){ pendingSessionAssetIds.clear(); }
        if(saveState){
      ...
      async function attachFilesToNode(targetNode, files){
        ...
        results.forEach((result,index)=>{
          ...
          if(result.status==='fulfilled'){
            ...
            if(!Number.isFinite(currentMapId) || currentMapId<=0){
              rememberPendingSessionAsset(uploaded.id);
            }else{
              forgetPendingSessionAsset(uploaded.id);
            }
            didChange=true;
          }else{
      ...
      function createAttachmentManager(){
        ...
        function backupAndRemoveAssetMeta(ids){
          ...
          ids.forEach(id=>{
            ...
            if(assetMeta.has(numeric)){
              ...
              forgetPendingSessionAsset(numeric);
            }
          });
          return stash;
        }
        ...
        function removeAttachmentsFromMind(ids){
          ...
          ids.forEach(forgetPendingSessionAsset);
          let changed=false;
        ...
        function onFilesUploaded(entries, targetNode){
          if(entries && entries.length){
            entries.forEach(entry=>{
              ...
              if(Number.isFinite(currentMapId) && currentMapId>0){
                forgetPendingSessionAsset(entry.id);
              }else{
                rememberPendingSessionAsset(entry.id);
              }
            });
          }
      ...
      window.addEventListener('beforeunload',e=>{
        ...
        const pendingIds=collectPendingSessionAssetIds();
        if(pendingIds.length){ purgePendingSessionAssets(pendingIds); }
