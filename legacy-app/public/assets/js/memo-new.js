(function(){
      ...
      const state = { id: 0 };
      const config=window.memoNewConfig||{};
      const UPLOAD_ACCEPT=config.uploadAccept||'';
      ...
      (async function bootstrap(){
        const res=await fetch(location.href,{method:'POST',headers:{'X-Requested-With':'XMLHttpRequest'},body:new URLSearchParams([['action','create_draft']])});
                ...
                <div style="margin-top:6px;display:grid;gap:6px">\
                  ...
                  <form onsubmit="return saveStepNotesAJAX(event, ${s.id})" id="form-notes-${s.id}">\
                    ...
                    <div style="display:flex;gap:8px;justify-content:space-between;align-items:center;margin-top:6px;flex-wrap:wrap">\
                      <div>\
                        <input id="att-file-step-${s.id}" type="file" accept="${UPLOAD_ACCEPT}" style="display:none">\
