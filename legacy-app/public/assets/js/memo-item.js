const memoItemConfig=window.memoItemConfig||{};
const UPLOAD_ACCEPT=memoItemConfig.uploadAccept||'';
const ITEM_ID=memoItemConfig.itemId!=null?String(memoItemConfig.itemId):'';
      (function(){
      ...
      if(itemFileInput){
        itemFileInput.addEventListener('change', async (e)=>{
          ...
          const fd=new FormData(); fd.append('action','upload_attachment'); fd.append('target','item'); fd.append('target_id', ITEM_ID); fd.append('file', f);
      ...
      function addStepForm(ev){
        ...
        fetch(location.href,{method:'POST',body:fd,headers:{'X-Requested-With':'XMLHttpRequest'}}).then(()=>location.reload());
        return false;
      }
      (function(){ // DnD steps
...
document.querySelectorAll('textarea[id^="md-step-"]').forEach(textarea=>{
  const id=textarea.id.replace('md-step-','');
  const preview=document.getElementById('step-md-view-'+id);
  if(preview){ preview.innerHTML=safeHTML(textarea.value||''); }
});
