...
const apiBase = new URL('api/', assetBase);
...
async function loadItems() {
  ...
  const data = await fetchJSON(`items?${params.toString()}`);
...
function renderItems() {
  ...
  for (const item of state.items) {
    ...
    doneInput.addEventListener('change', async () => {
      try {
        await fetchJSON(`items/${item.id}/done`, {
          method: 'PATCH',
    ...
    deleteBtn.addEventListener('click', async () => {
      ...
      try {
        await fetchJSON(`items/${item.id}`, { method: 'DELETE' });
        await Promise.all([loadCategories(), loadItems()]);
      } catch (error) {
        handleError(error);
      }
    ...
    addStepBtn.addEventListener('click', async () => {
      ...
      if (!title) return;
      try {
        await fetchJSON(`items/${item.id}/steps`, {
          method: 'POST',
    ...
    (item.steps || []).forEach((step) => {
      ...
      stepDone.addEventListener('change', async () => {
        try {
          await fetchJSON(`steps/${step.id}/done`, {
            method: 'PATCH',
      ...
      deleteStepBtn.addEventListener('click', async () => {
        try {
          await fetchJSON(`steps/${step.id}`, { method: 'DELETE' });
          await loadItems();
        } catch (error) {
          handleError(error);
        }
